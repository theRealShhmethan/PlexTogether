...
- **Drift correction (tuned after testing):** the brief's thresholds (ignore under 250 ms, hard seek over 2 s) made both players buffer non-stop. Every Plex reposition is a new transcode session, and the NAS couldn't keep up. Now: under **3 s** ignore; 3–8 s playback rate ±4% (speeding up only with ≥15 s buffered, so it never outruns the transcoder); over 8 s reposition, **at most once per 30 s**, preferring an instant seek within the buffer. Nothing is corrected while buffering or for 10 s after a reposition.
- **Buffering pauses the room:** if anyone's player buffers for more than 1.5 s while the room plays, the server pauses the room at *that player's* position ("Waiting for Lexi…"). When everyone it's waiting for has recovered, it resumes everyone together 1.5 s ahead. A deliberate play/pause/seek overrides the wait. Status reports carry each player's position and are sent immediately when buffering starts or stops.
